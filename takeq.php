        ...
        if ($res) {
            ...
            if ($count == 0) {
                ...
                while ($row = mysqli_fetch_assoc($res)) {
                    $options = array($row["op1"], $row["op2"], $row["op3"], $row["answer"]);
                    shuffle($options); // Shuffle the options array to randomize order
    
                    echo $i . ". " . $row["qs"] . "<br>";
                    for ($j = 0; $j < 4; $j++) {
                        echo "<input class=\"\" type=\"radio\" value=\"" . $j . "\" name=\"ans" . $i . "\">" . $options[$j] . "<br>";
                    }
    
                    // Store the correct answer index in a hidden field
                    echo "<input type=\"hidden\" name=\"correct_ans" . $i . "\" value=\"" . array_search($row["answer"], $options) . "\">";
                    $i++;
                }
        ...
        if (isset($_POST["submit"])) {
            for ($i = 1; $i <= $count; $i++) {
                if (isset($_POST["ans" . $i])) {
                    ...
                    $correct_option = $_POST["correct_ans" . $i];
